...
// モックデータ（Figmaのデザインを再現するためのサンプルデータ）
const MOCK_LTS = [
  { id: 1, title: 'Next.js 14のServer Actionsについて', speaker: '山田 太郎', date: '2026-07-01', category: 'FRONTEND', summary: '新しいデータ更新手法についての解説と実践的なコード例を紹介します。' },
  { id: 2, title: 'Supabaseを利用したRLSセキュリティ設計', speaker: '佐藤 次郎', date: '2026-06-25', category: 'INFRASTRUCTURE', summary: '行レベルセキュリティの基本から、チーム開発でのベストプラクティスを共有。' },
  { id: 3, title: 'LLMを活用した社内業務効率化ツール開発', speaker: '鈴木 花子', date: '2026-06-12', category: 'MACHINE LEARNING', summary: 'OpenAI APIとLangChainを組み合わせたプロトタイプ開発の裏側。' },
  { id: 4, title: 'Tailwind CSSによるコンポーネント設計', speaker: '高橋 健太', date: '2026-05-30', category: 'FRONTEND', summary: '共通デザインシステムを効率よく実装するためのTips集です。' },
  { id: 5, title: 'Dockerでのローカル開発環境構築自動化', speaker: '田中 実', date: '2026-04-15', category: 'INFRASTRUCTURE', summary: '新規メンバーが1コマンドで環境構築できる仕組みを作りました。' },
];
...
export default function LtListPage() {
  ...
  // 2. ソート処理（日付順、または年度順 ※モックでは簡易的に日付で並び替え）
  const sortedLts = useMemo(() => {
    return [...filteredLts].sort((a, b) => {
      if (sortBy === 'date') {
        return new Date(b.date).getTime() - new Date(a.date).getTime(); // 最新順
      } else {
        return new Date(a.date).getTime() - new Date(b.date).getTime(); // 古い順（年度順の簡易表現）
      }
    });
  }, [filteredLts, sortBy]);

  // 最新の数件（カード用：上位3件）と、それ以外（リスト用）に分離
  ...
  return (
    <div className="mx-auto max-w-[1000px] px-6 py-4 h-full flex flex-col justify-between text-gray-800">
      ...
      <div className="flex items-center justify-between border-b border-gray-200 pb-2 mb-3">
        ...
        <Link
          className="inline-flex h-9 items-center justify-center rounded-md bg-[#0f4c9c] px-4 text-sm font-medium text-white hover:bg-[#0c3d7e] transition-all"
          href="/lt/create"
        >
          ＋ 新規作成
        </Link>
            ...
            <div key={lt.id} className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm flex flex-col justify-between h-[130px]">
              <div>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className="text-[11px] text-gray-400">{lt.date}</span>
                </div>
            ...
            <tbody className="divide-y divide-gray-100">
              {listItems.map((lt) => (
                <tr key={lt.id} className="hover:bg-gray-50 transition-colors">
                  <td className="p-2.5 text-xs text-gray-500 whitespace-nowrap">{lt.date}</td>
